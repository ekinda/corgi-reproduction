"""Imputation evaluation on chr1/chr10 bins for multiple models.

Builds a single NPZ containing per-tissue, per-channel 1D arrays for each model:
- mean_baseline
- corgi_avg_tf
- corgi_tissue_tf
- corgiplus_rna_qn
...
Bins are 64bp bins on chr1/chr10 overlapping merged regions from
hg38_sequence_folds_tfexcluded34.bed. Outputs are ordered as chr1 bins
followed by chr10 bins.
"""
...
def main() -> None:
    ...
    # Avocado handled in a separate script (older Python env).
    ...
    for chrom in ("chr1", "chr10"):
        chrom_bin_count = len(selected_bins[chrom])
        logging.info("Selected %d bins for %s (%d bp)", chrom_bin_count, chrom, chrom_bin_count * BIN_SIZE)
    logging.info("Total selected bins: %d (%d bp)", total_bins, total_bins * BIN_SIZE)
    bin_chrom = []
    bin_start = []
    bin_end = []
    bin_index = []
    ...
    for chrom in ("chr1", "chr10"):
        mapped = int((out_index[chrom] >= 0).sum())
        logging.info(
            "Out-index coverage %s: mapped=%d / bins=%d (%.3f)",
            chrom,
            mapped,
            chrom_bins[chrom],
            mapped / chrom_bins[chrom] if chrom_bins[chrom] > 0 else 0.0,
        )
    ...
    for chrom in ("chr1", "chr10"):
        corgi_windows_by_chrom[chrom] = windows_from_intervals(
            interval_bins[chrom], CORGI_TARGET_BINS, chrom_bins[chrom]
        )
    total_corgi_windows = sum(len(corgi_windows_by_chrom[chrom]) for chrom in corgi_windows_by_chrom)
    logging.info("Total 524kb sequences per tissue: %d", total_corgi_windows)
    logging.info("Total 524kb sequences overall: %d", total_corgi_windows * len(tissues))

    results: Dict[str, np.ndarray] = {}

    logging.info("Starting imputation over %d tissues", len(tissues))
    ...
    for tissue_id in tissues:
        ...
        # Pre-allocate output arrays per channel/model
        ...
        # Corgi-family models
        logging.info("[tissue %d] Corgi-family start", tissue_id)
        for chrom in ("chr1", "chr10"):
            chrom_bins_n = chrom_bins[chrom]
            out_idx = out_index[chrom]
            corgi_windows = corgi_windows_by_chrom[chrom]
            logging.info(
                "[tissue %d] %s corgi windows: %d", tissue_id, chrom, len(corgi_windows)
            )
            ...
            for start_bin in corgi_windows:
                ...
                if n_count > 0:
                    logging.info(
                        "Found %d ambiguous bases in %s:%d-%d",
                        n_count,
                        chrom,
                        start_bp,
                        end_bp,
                ...
                # RNA tracks + mean baseline (8192 bins)
        ...
        # Epigept
        logging.info("[tissue %d] Epigept start", tissue_id)
        for chrom in ("chr1", "chr10"):
            chrom_bins_n = chrom_bins[chrom]
            out_idx = out_index[chrom]
            intervals = interval_bins[chrom]
            epi_windows = windows_from_intervals(intervals, EPIGEPT_OUT_BINS_64, chrom_bins_n)
            logging.info(
                "[tissue %d] %s epigept windows: %d", tissue_id, chrom, len(epi_windows)
            )
            ...
            for start_bin in epi_windows:
                ...
                dna_onehot = one_hot_encode(seq)
                epi_pred = epigept_wrapper(dna_onehot, tf_expr_epi[tissue_id]).cpu().numpy()  # (1000, 22)
                epi_pred_64 = np.repeat(epi_pred, 2, axis=0)  # (2000, 22)
